...
| AddOns, WTF, Fonts, Interface resources | `src/core/app/config/tests.rs::config_service_plans_and_applies_shareable_package_with_mapping_backup_and_rewrite` verifies AddOns cleanup/write, `WTF/Config.wtf`, root, account, and character SavedVariables, Fonts, and Interface resources. `src/core/bundle/tests/external_package/analysis.rs::analyze_external_package_directory_detects_direct_addons_and_root_savedvariables` covers root-level `WTF/SavedVariables` layouts. | Covered by integration test | Synthetic fixture plus root-layout regression coverage, but it exercises all required resource groups through the shared app facade. |
...
| Lua SavedVariables safe rewrite | `src/core/lua_patch/tests/bytes/{fixtures,boundary,scope,encoding}.rs`, `src/core/lua_patch/tests/text.rs`, and `src/core/lua_patch/testdata/FIXTURES.md` cover scoped profile keys, identity fields, known identity-key containers, UTF-8, invalid UTF-8, Latin-1, and controlled reductions from aggregate local shape findings. Fresh targeted run: `CARGO_BUILD_JOBS=1 cargo nextest run lua_patch -j 1` passed 40/40. | Improved but still not complete for broad claims | New fixtures added for `AuraUpdater.lua`, `DBM-*`, `Details_*.lua`, `EventsTracker.lua`, `ExWindCore.lua`, `HandyNotes_*.lua`, `MeetingStone.lua`, `SavedInstances.lua`, `TinyTooltip-Remake.lua`, `WeakAuras.lua`, `WeakAurasArchive.lua`, `WorldQuestTracker.lua`, and `ZygorGuidesViewer.lua`, plus controlled reductions for `DBM-Core`, `HandyNotes_Dragonflight`, `MeetingStone`, `SavedInstances`, and `WorldQuestTracker`. The manifest records coverage expectations, fail-closed intent, and privacy-preserving provenance notes; broader real-package reductions are still missing. |
...
- Retried with reduced concurrency:
  ...
  - Result after the new fixtures: 40 tests run, 40 passed, 725 skipped.
...
- `CARGO_BUILD_JOBS=1 cargo nextest run -j 1` passed: 766 tests run, 766 passed, 0 skipped.
...
3. The current audit has now performed read-only local structure scans and a read-only live
   `config inspect` pass against `E:\Games\World of Warcraft\_retail_`. It still did not ingest,
   sanitize, or run the apply pipeline against those live contents. That is acceptable for privacy,
