...
- [x] remove remaining thin-forwarder service behavior by moving real normalization and policy
  ...
  and external-package flows doing so. `StableAppServices` now owns the first-wave stable direct/
    ...
    Current cleanup: addon-lock CLI output now also follows the focused renderer layout. Plan/apply,
    ...
    Current cleanup: bundle response contracts now split manifest/resource summaries, archive
    inspect/pack results, local account and character mapping DTOs, apply plan/apply DTOs, and
    embedded addon-lock bridge results under focused `response::bundle::*` child modules.
    ...
    Current cleanup: `core::app` no longer carries dead-code allowances. Test-only custom provider
    injection and value round-trip helpers are explicitly `#[cfg(test)]`, while production app
    contracts expose only the runtime and projection paths they actually support.
...
- [ ] tighten any remaining path portability edge cases around case folding, archive metadata, and
  ...
  Current progress: runtime policy is now immutable after build. Post-build `AppRuntime` mutators
  ...
  Current progress: bundle apply mappings now also validate at the domain boundary. Mapping files,
  app DTOs, and direct planning inputs reject duplicate selected accounts and overlapping
  character override rows before planning.
  ...
  Current progress: addon-lock source sidecars now reject empty source lists, blank or
  whitespace-padded comparison keys, non-portable source paths, and case-insensitive duplicate
  source paths before source override resolution.
  ...
  transport helpers.
  Current cleanup: GitHub provider internals now follow the same split. The provider root keeps
  release fetch, release/asset selection, cache-validator projection, and transport headers, while
