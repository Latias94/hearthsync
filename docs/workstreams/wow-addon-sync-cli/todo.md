...
- [ ] Remove dead or duplicated transition paths after planner/executor boundaries are stable
  ...
  - Current cleanup: bundle root module now separates archive constants and public API exports into `core::bundle::{constants,exports}`, and the transitional `core::bundle::imports` prelude has been retired after owner-module imports replaced its remaining call sites.
  ...
  - Current cleanup: `core::bundle::{archive_read,apply_policy,target_accounts}` now act only as parent-visible namespace modules, and callers import concrete child modules such as `archive_read::inspect`, `apply_policy::cleanup`, and `target_accounts::selection` explicitly instead of depending on hidden function-level re-export shells.
  - Current cleanup: `core::bundle::{shared,apply_model,execution,wtf_archive}` now also act only as parent-visible namespace modules, and bundle internals now import concrete child modules such as `shared::path`, `shared::zip_options`, `apply_model::prepared`, `execution::compare`, and `wtf_archive::resolve` explicitly instead of routing through shell-level type/function re-exports.
  - Current cleanup: `core::bundle::{addon_source_archive,apply_source,entry_plan,planner}` now also avoid shell-level internal re-exports; bundle code imports `addon_source_archive::{index_paths,lock,source_bundle}`, `apply_source::reader`, `entry_plan::context`, and `planner::pipeline` directly where those seams are actually used.
